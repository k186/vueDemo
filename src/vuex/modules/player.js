...
const actions = {
  ...
  playerPause ({commit}) {
    ...
    let PlayerComp = {
      currentPlay: {
        audio: state.PlayerComp.currentPlay.audio,
        duration: state.PlayerComp.currentPlay.audio.duration,
        currentTime: state.PlayerComp.currentPlay.audio.currentTime
        /* set currentTime */
      }
    }
  ...
  playerSet ({commit, dispatch}, {option}) {
    ...
    function updatePlayList (currentPlayList, historyList, initOrder, callback) {
      let playList = {
        currentPlayList: currentPlayList !== null ? currentPlayList : {count: 0, sheetCode: '', list: [], title: ''},
        historyList: historyList !== null ? historyList : {count: 0, sheetCode: '', list: [], title: ''},
        initOrder: initOrder === true ? initOrder : false
      }
    ...
    if (option.from === 'list') {
      ...
      dispatch('updateRadioToPlayList', {
        callback: function () {
          if (!isPlayCurrent(option.uid)) {
            playPause()
            setCurrent()
      ...
      dispatch('setFavouriteToPlayList', {
        callback: function () {
          if (!isPlayCurrent(option.uid)) {
            playPause()
            setCurrent()
  ...
  updatePlayOrderIndex ({commit}, {playIndexObj}) {
    let playIndex = playIndexObj != null ? playIndexObj : {playOrderIndex: state.PlayerComp.playOrder.indexOf(state.PlayerComp.currentPlay.uid)}
